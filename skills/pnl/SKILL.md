...
Load these instructions with `load_skill(name="pnl")`. Read both references using
`load_skill_reference(name="pnl", reference="dataset")` and `reference="policy"`;
...
`execute_review_analysis(skill_name="pnl", source_paths=[...])` with all assigned
