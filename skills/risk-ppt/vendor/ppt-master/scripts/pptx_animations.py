...
def _load_native_animations() -> dict[str, dict[str, Any]]:
    ...
    if not isinstance(raw_effects, list):
        raise RuntimeError(
            'native animation preset manifest field "effects" must be a list'
        )
...
_NODE_TYPE_TRIGGERS = {value: key for key, value in _TRIGGER_NODE_TYPES.items()}
...
def _normalize_animation_color(value: object, field: str) -> str:
    if not isinstance(value, str):
        raise ValueError(f"{field} must be #RRGGBB or theme:<scheme-color>: {value!r}")
...
def normalize_animation_effect_options(
    ...
    if missing_required:
        ...
        raise ValueError(f"animation effect {effect!r} requires {required_fields}")
    ...
    for name, value in options.items():
        ...
        if option_type == "enum":
            ...
            normalized[name] = (
                int(key) if name == "amount" and re.fullmatch(r"\d+", key) else key
            )
            ...
            raise AssertionError(
                f"unhandled animation effect option type: {option_type}"
            )
    return normalized
...
def _normalize_sound(value: object) -> tuple[str | None, str | None]:
    ...
    if not isinstance(value, Mapping):
        raise ValueError(
            "low-level animation sound must be an object with relationship_id and name"
        )
    ...
    if not isinstance(name, str) or not name.strip():
        raise ValueError(f"low-level animation sound name must be non-empty: {name!r}")
...
def _normalize_target_mapping(
    ...
    if trigger_shape_id == shape_id:
        raise ValueError("animation trigger_shape_id must target a different shape")
    target_trigger = (
    ...
    if trigger_shape_id is not None:
        if "trigger" in target and target_trigger != "on-click":
            raise ValueError(
                'animation target with trigger_shape_id must use trigger "on-click"'
            )
    ...
    if (
        ...
        raise ValueError("animation accelerate + decelerate must not exceed 1")
    bounce_end = (
    ...
    if bounce_end and decelerate:
        raise ValueError(
            "animation bounce_end and decelerate are mutually exclusive in PowerPoint"
        )
...
def _normalize_target(
    ...
    if isinstance(target, (str, bytes)) or not isinstance(target, Sequence):
        raise ValueError(
            f"animation target must be a 3- or 4-item sequence: {target!r}"
        )
...
_MIXED_POOL = [
    "entrance_blinds",
    "entrance_checkerboard",
    "entrance_dissolve",
    "entrance_fly",
    "entrance_ascend",
    "entrance_random_bars",
    "entrance_box",
    "entrance_split",
    "entrance_strips",
    "entrance_wedge",
...
_SEMANTIC_PATTERNS: list[tuple[tuple[str, ...], str]] = [
    ...
    (
        ("takeaway", "callout", "quote", "source", "conclusion", "note", "try-at-home"),
        "entrance_fade",
    ),
]
...
def _row_timing_profile(
    ...
    if total is None or total <= 1:
        return ()
    parent_map = {child: parent for parent in row.iter() for child in list(parent)}
    behavior_names = {
        "anim",
        "animClr",
        "animEffect",
...
def _set_effect_option_value(
    ...
    if name == "amount" and animation == "emphasis_transparency":
        ...
        for node in row.iter(_qn(PML_NS, "set")):
            ...
            matched = True
        if not matched:
            raise RuntimeError("emphasis_transparency preset lost its opacity behavior")
        return
    if name == "color":
    ...
    if name == "font_name":
        ...
        for node in row.iter(_qn(PML_NS, "set")):
            ...
            values = list(node.iter(_qn(PML_NS, "strVal")))
            if len(values) != 1:
                raise RuntimeError("emphasis_change_font preset lost its font value")
            values[0].set("val", str(value))
            matched = True
        if not matched:
            raise RuntimeError("emphasis_change_font preset lost its font behavior")
        return
    if name == "relative":
    ...
    if name == "size":
        ...
        if len(scales) != 1:
            raise RuntimeError("emphasis_grow_shrink preset lost its p:animScale node")
        amount = str(round(float(value) * 1000))
        targets = scales[0].findall(_qn(PML_NS, "to"))
        if len(targets) > 1:
            raise RuntimeError("emphasis_grow_shrink preset lost its scale value")
        target = targets[0] if targets else ET.SubElement(scales[0], _qn(PML_NS, "to"))
        target.set("x", amount)
        target.set("y", amount)
        return
...
def _append_after_effect(
    ...
    ctn_attributes = {
        ...
        "fill": "hold",
        "display": "0",
        "masterRel": ("sameClick" if target.after_effect == "hide" else "nextClick"),
        "afterEffect": "1",
    }
...
def _main_target_offsets(targets: Sequence[AnimationTarget]) -> list[int]:
    ...
    for target in targets:
        if target.trigger == "on-click":
            ...
            start_ms = (previous_start_ms if has_previous else 0) + target.delay_ms
        else:
            start_ms = (
                previous_start_ms + previous_duration_ms if has_previous else 0
            ) + target.delay_ms
...
def create_sequence_timing_xml(
    ...
    if not targets:
        return ""
    normalized_targets = [
        _normalize_target(target, default_dur_ms, trigger) for target in targets
    ]
    next_id = 3
    main_targets = [
        target for target in normalized_targets if target.trigger_shape_id is None
    ]
    interactive_targets = [
        target for target in normalized_targets if target.trigger_shape_id is not None
    ]

    main_triggers = {target.trigger for target in main_targets}
    main_trigger = next(iter(main_triggers)) if len(main_triggers) == 1 else None
    needs_per_target_layout = main_trigger is None or (
...
def pick_animation_effect(
    ...
    *,
    ...
    if isinstance(offset, bool) or not isinstance(offset, int) or offset < 0:
        raise ValueError(f"animation offset must be a non-negative integer: {offset!r}")
...
def _direct_conditions(ctn: ET.Element) -> list[ET.Element]:
    ...
    if condition_list is None:
        return []
    return [child for child in list(condition_list) if child.tag == _qn(PML_NS, "cond")]
...
def _shape_index(
    ...
    parent_map = {
        child: parent for parent in slide_root.iter() for child in list(parent)
    }
...
def _row_filter(
    ...
    if (
        ...
        and effect.get("transition") != expected_transition
    ):
        errors.append(
            f'{preset_class} p:animEffect must set transition="{expected_transition}"'
        )
    ...
    for name, spec in option_specs.items():
        ...
        if option_type == "enum":
            ...
            for value, variant_xml in spec["values"].items():
                variant = ET.fromstring(variant_xml)
                variant_filter = _row_filter(
                    variant, variant.get("presetClass", ""), []
                )
            ...
            for node in row.iter(_qn(PML_NS, "set")):
                ...
                if "style.opacity" in attributes:
                    opacity_values.extend(
                        value.get("val") for value in node.iter(_qn(PML_NS, "strVal"))
                    )
            if len(opacity_values) != 1:
                ...
                try:
                    ...
                    errors.append("emphasis_transparency row has a non-numeric opacity")
        elif name == "color":
            ...
            if len(fonts) != 1:
                errors.append("emphasis_change_font row must contain one font name")
                continue
            try:
            ...
            scales = list(row.iter(_qn(PML_NS, "animScale")))
            targets = scales[0].findall(_qn(PML_NS, "to")) if len(scales) == 1 else []
            raw = (
                targets[0].get("x")
                if len(targets) == 1
                else "100000"
                if not targets and len(scales) == 1
                else None
            )
            if raw is None or not re.fullmatch(r"\d+", raw):
            ...
            errors.append(f"animation effect {effect!r} option {name!r} has no reader")
    return values
...
def _timing_summary(
    ...
    if len(raw_bounce_values) > 1:
        errors.append("object-animation behaviors disagree on p14:bounceEnd")
    elif raw_bounce_values:
        raw_bounce = next(iter(raw_bounce_values))
        if raw_bounce is None or not re.fullmatch(r"\d+", raw_bounce):
            errors.append(
                f"object-animation p14:bounceEnd must be numeric; found {raw_bounce!r}"
            )
        else:
...
def _after_effect_summary(
    ...
    after_nodes = [
        ...
        if any(ctn.get("afterEffect") == "1" for ctn in node.iter(_qn(PML_NS, "cTn")))
    ]
    if not after_nodes:
        return "none", None
    if len(after_nodes) != 1:
        errors.append(f"object-animation row contains {len(after_nodes)} after effects")
        return "none", None
    node = after_nodes[0]
...
def _row_offset_ms(
    ...
    leaf_delay = None
    if len(leaf_conditions) == 1 and re.fullmatch(
        r"\d+", leaf_conditions[0].get("delay") or ""
    ):
        leaf_delay = int(leaf_conditions[0].get("delay", "0"))
    else:
        errors.append("object-animation row must have one numeric leaf start condition")
    current = parent_map.get(row)
    ...
    if trigger == "on-click" and trigger_shape_id is None and not saw_indefinite:
        errors.append(
...
def _row_matches_powerpoint_behavior(
    ...
    *,
    ...
    option_candidates.extend(
        {name: value for name, value in effect_options.items() if name != omitted}
...
def _select_supported_timing_branch(slide_root: ET.Element) -> ET.Element:
    ...
    for index, child in list(enumerate(list(projected))):
        if child.tag != alternate_tag:
            continue
        branches = [
            branch for branch in list(child) if branch.tag in {choice_tag, fallback_tag}
        ]
...
def validate_slide_animation_structure(
    ...
    *,
    ...
    direct_timings = [
        child for child in list(slide_root) if child.tag == _qn(PML_NS, "timing")
    ]
    ...
    extension_list = next(
        (child for child in root_children if child.tag == _qn(PML_NS, "extLst")),
        None,
    )
    if (
        extension_list is not None
        and root_children.index(extension_list) < timing_index
    ):
    ...
    parent_map = {child: parent for parent in timing.iter() for child in list(parent)}
    ctn_ids: list[int] = []
    for ctn in timing.iter(_qn(PML_NS, "cTn")):
        if ctn.get("id") is None:
            ...
            if ancestor is not None and ancestor.tag == _qn(PML_NS, "subTnLst"):
                continue
        value = _int_attribute(
            ctn,
            "id",
    ...
    duplicates = sorted(value for value in set(ctn_ids) if ctn_ids.count(value) > 1)
    if duplicates:
        errors.append("duplicate p:cTn@id values: " + ", ".join(map(str, duplicates)))
    ...
    if require_supported_effects:
        ...
        rows = []
    if rows:
        regular_rows = [row for row in rows if row.trigger_shape_id is None]
        interactive_rows = [row for row in rows if row.trigger_shape_id is not None]
        main_sequences = [
            node
            for node in timing.iter(_qn(PML_NS, "cTn"))
        ...
        for row in rows:
            if row.trigger_shape_id is not None and row.trigger != "on-click":
                errors.append(
...
def read_slide_animation_sequence(
    ...
    *,
    ...
    direct_timings = [
        child for child in list(root) if child.tag == _qn(PML_NS, "timing")
    ]
    ...
    regular_triggers = {row.trigger for row in rows if row.trigger_shape_id is None}
    trigger = (
...
def validate_generated_animation_xml(
    ...
    *,
    ...
    normalized_expected = tuple(
        _normalize_target(target, default_duration_ms, trigger) for target in targets
    )
    ...
    expected = tuple(
        target for target in normalized_expected if target.trigger_shape_id is None
    ) + tuple(
        target for target in normalized_expected if target.trigger_shape_id is not None
    )
    ...
    expected_main_triggers = {target.trigger for target in expected_main_targets}
    ...
    for index, (actual, target) in enumerate(zip(summary.rows, expected), 1):
        ...
        if index <= len(actual_row_elements):
            ...
            expected_behavior_row.set(
                "id",
                actual_row_id if actual_row_id and actual_row_id.isdigit() else "1",
            )
            _append_after_effect(
            ...
            if not _animation_spec_matches_row(
                ...
                errors.append(
                    f"animation row {index} PowerPoint-authored behavior tree changed"
                )
        ...
        expected_row_trigger = target.trigger
    ...
    try:
        with zipfile.ZipFile(path) as package:
            ...
            for name in names:
                ...
                rels_name = f"ppt/slides/_rels/{slide_leaf}.rels"
                if rels_name not in package.namelist():
                    errors.append(f"{name}: animation sound relationships are missing")
                    continue
                try:
                ...
                for relationship_id in sorted(sound_ids):
                    ...
                    if target_part not in package.namelist():
                        errors.append(
                            f"{name}: animation sound target is missing: {target_part}"
                        )
...
def object_animation_fingerprint(slide_xml: str | bytes) -> str | None:
    ...
    try:
        ...
        raise ValueError(f"invalid slide XML: {exc}") from exc
    root = _select_supported_timing_branch(root)
    timings = [child for child in list(root) if child.tag == _qn(PML_NS, "timing")]
    if len(timings) > 1:
        raise ValueError(
    ...
    has_object_animation = any(
        ...
        or (
            element.tag == _qn(PML_NS, "cTn") and element.get("presetClass") is not None
        )
...
def describe_animation_effect(effect: object) -> dict[str, Any]:
    ...
    implied_options = (
        dict(ANIMATION_ALIAS_OPTIONS.get(effect, {})) if isinstance(effect, str) else {}
    )
    option_contract: dict[str, Any] = {}
    for name, raw_spec in NATIVE_ANIMATIONS[canonical]["effectOptions"].items():
        spec = {key: value for key, value in raw_spec.items() if key != "values"}
    ...
    return {
        ...
        "timing": {
            "duration": ("positive seconds; legacy group effect or effects[] row"),
            "delay": ("non-negative seconds; legacy group effect or effects[] row"),
...
def effective_animation_effect_options(
    ...
    if errors:
        raise RuntimeError(
            f"animation effect {canonical!r} option model failed: " + "; ".join(errors)
        )
    return effective
