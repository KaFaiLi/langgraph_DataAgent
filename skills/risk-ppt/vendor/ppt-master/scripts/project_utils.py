...
def parse_project_name(dir_name: str) -> Dict[str, str]:
    ...
    full_match = re.match(
        r"^(?P<name>.+)_(?P<format>[a-z0-9_-]+)_(?P<date>\d{8})$", dir_name_lower
    )
    if full_match:
    ...
    *,
    ...
    if check_lock:
        ...
        if communication_match is None:
            ...
            communication_block = lock_text[communication_match.end() : section_end]
            missing_keys = [
                key
    ...
    if outline_match is None:
        errors.append(
            "Communication trace: design_spec.md must contain ## IX. Content Outline.",
        )
        return errors
    next_section = re.search(
        r"^##[ \t]+",
...
def validate_svg_viewbox(
    ...
    if len(distinct) > 1:
        details = ", ".join(
            f"{name}={viewbox.canonical}" for name, viewbox in sorted(viewboxes.items())
        )
        warnings.append("All project SVG root viewBoxes must match; found " + details)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Inspect and validate a PPT Master project."
    )
