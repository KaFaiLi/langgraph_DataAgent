...
def is_static_page_frame(
    ...
    return _normalized_marker(placeholder) is not None or is_chrome_role(role)
...
def validate_semantic_markers(
    ...
    *,
    ...
    duplicates = sorted(
        elem_id for elem_id in marked_ids if id_counts.get(elem_id, 0) > 1
    )
    if duplicates:
        issues.append(
            SemanticMarkerIssue(
                "error",
                "semantic markers require unique ids; duplicate: "
                + ", ".join(duplicates),
            )
        )
    return issues
