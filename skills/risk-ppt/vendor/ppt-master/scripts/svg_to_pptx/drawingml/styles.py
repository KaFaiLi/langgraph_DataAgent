...
from .utils import (
    ...
    resolve_url_id,
    parse_project_filter_params,
    project_filter_drawingml_coordinates,
...
def build_gradient_fill(
    ...
    for child in grad_elem:
        ...
        if effective_opacity is not None:
            alpha_xml = f'<a:alpha val="{quantize_ooxml_alpha(effective_opacity)}"/>'
    ...
    if tag == "linearGradient":
        x1 = parse_project_linear_gradient_coordinate(grad_elem.get("x1", "0"))
        y1 = parse_project_linear_gradient_coordinate(grad_elem.get("y1", "0"))
        x2 = parse_project_linear_gradient_coordinate(grad_elem.get("x2", "1"))
        y2 = parse_project_linear_gradient_coordinate(grad_elem.get("y2", "0"))
...
def build_pattern_fill(
    ...
    fallback_fg = next(
        (entry for entry in paint_entries if entry["attr"] == "stroke"), None
    )
...
def build_shadow_xml(
    ...
    alpha_val = quantize_ooxml_alpha(p["opacity"] * opacity_multiplier * 0.75)
...
def get_stroke_opacity(
    ...
    stroke_op = (
        _get_attr(elem, "stroke-opacity", ctx) if ctx else elem.get("stroke-opacity")
    )
