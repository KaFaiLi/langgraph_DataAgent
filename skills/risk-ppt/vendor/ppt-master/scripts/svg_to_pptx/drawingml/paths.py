...
_PATH_NUMBER_PATTERN = r"[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?"
...
_CANONICAL_FREEFORM_NUMBER_RE = re.compile(r"-?(?:\d+(?:\.\d+)?|\.\d+)$")
...
def _tokenize_path_data(d: str) -> list[_GeometryToken]:
    ...
    for match in _PATH_TOKEN_RE.finditer(d):
        ...
        if "," in gap and (
            previous_kind is None or previous_kind == "command" or kind == "command"
        ):
            raise ValueError(f"path d has a misplaced comma at offset {cursor}")
...
def _expand_arc_argument_tokens(
    ...
    while pending:
        ...
        if position in {3, 4}:
            ...
            expanded.append(_GeometryToken("number", token.raw[0], token.offset))
            remainder = token.raw[1:]
            if remainder:
                next_position = (position + 1) % _ARG_COUNTS[command_token.raw]
                if (next_position in {3, 4} and remainder[0] not in {"0", "1"}) or (
...
def project_gradient_geometry_errors(root: ET.Element) -> list[str]:
    ...
    for elem in root.iter():
        ...
        zero_width = math.isclose(min_x, max_x, rel_tol=0.0, abs_tol=1e-9)
        zero_height = math.isclose(min_y, max_y, rel_tol=0.0, abs_tol=1e-9)
        if not zero_width and not zero_height:
            continue

        dimension = (
            "width and height"
            if zero_width and zero_height
            else ("width" if zero_width else "height")
        )
        elem_id = elem.get("id")
        label = f"<{tag} id={elem_id!r}>" if elem_id else f"<{tag}>"
        errors.add(
