...
def _require_native_marker_attributes(
    ...
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid native replacement metadata: {preview}{suffix}"
    )
...
def _require_project_freeform_geometry(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project freeform geometry: {preview}{suffix}"
    )
...
def _require_project_nested_svg_crops(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid nested SVG crop wrapper(s): {preview}{suffix}"
    )
...
def _require_project_clip_paths(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project clip-path(s): {preview}{suffix}"
    )
...
def _require_project_transforms(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project transform(s): {preview}{suffix}"
    )
...
def _require_project_stroke_styles(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project line style(s): {preview}{suffix}"
    )
...
def _require_project_opacities(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project opacity value(s): {preview}{suffix}"
    )
...
def _require_project_paints(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project paint value(s): {preview}{suffix}"
    )
...
def _require_project_masks(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project mask(s): {preview}{suffix}"
    )
...
def _require_project_definitions(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project definition(s): {preview}{suffix}"
    )
...
def _require_project_paint_references(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project paint reference(s): {preview}{suffix}"
    )
...
def _require_project_line_end_markers(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project line-end marker(s): {preview}{suffix}"
    )
...
def _require_project_gradients(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project gradient(s): {preview}{suffix}"
    )
...
def _require_project_filters(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project filter(s): {preview}{suffix}"
    )
...
def _require_project_effect_status(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: unsupported imported PPTX effect(s): {preview}{suffix}"
    )
...
def _require_project_text_properties(
    ...
    suffix = "" if len(errors) <= 8 else f"; +{len(errors) - 8} more"
    raise SvgNativeConversionError(
        f"{Path(svg_path).name}: invalid project text property(s): {preview}{suffix}"
    )
...
def _decode_unchanged_txbody(
    ...
    *,
    ...
    snapshot = group.get(_TXBODY_UNCHANGED_ATTR) if trust_runtime_snapshot else None
    unchanged = snapshot == "1" or (
        snapshot != "0" and svg_text_fingerprint(group) == expected_hash
    )
...
def _append_shape_text(
    ...
    if not shape.xml.lstrip().startswith("<p:sp>") or not shape.xml.rstrip().endswith(
        "</p:sp>"
    ):
        raise SvgNativeConversionError("Native txBody can only attach to p:sp")
    closing = shape.xml.rfind("</p:sp>")
    return ShapeResult(
        xml=(shape.xml[:closing] + txbody_xml + "\n" + shape.xml[closing:]),
...
def preserved_native_text_body(
    ...
    *,
    ...
    carrier_children = [
        child for child in group if child.get("data-pptx-part") == "geometry"
    ]
    allowed_parts = {
        "geometry",
        "geometry-detail",
...
def convert_g(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    should_animate_group = (
        ...
        and (
            not is_chrome
            or (not has_explicit_semantics and elem_id in ctx.animation_group_overrides)
        )
    ...
    unit_axis_reflection = bool(transform) and _is_unit_axis_reflection(
        parse_transform_operations(transform)
    )
    matrix_supported = (
        not native_subtree_active
        and bool(transform)
        and visual_children
        and (supports_full_project_transform(elem) or unit_axis_reflection)
    )
    ...
    logical_native_shape_group = elem.get("data-pptx-object") in {
        "shape",
        "connector",
    } and (
    ...
    logical_picture_effect_group = filter_id is not None and is_picture_effect_carrier(
        elem
    )
    ...
    if rotate_pivot is not None and angle_deg:
        ...
        delta_x = (
            (bbox_cx - pivot_ex) * cos_t
            - (bbox_cy - pivot_ey) * sin_t
            + pivot_ex
            - bbox_cx
        )
        delta_y = (
            (bbox_cx - pivot_ex) * sin_t
            + (bbox_cy - pivot_ey) * cos_t
            + pivot_ey
            - bbox_cy
        )
...
def _build_source_shape_id_map(root: ET.Element) -> dict[tuple[str, str], int]:
    ...
    for elem in root.iter():
        ...
        if re.fullmatch(r"[A-Za-z0-9_.-]{1,64}", scope) is None:
            raise SvgNativeConversionError(f"Invalid data-pptx-shape-scope {scope!r}")
        try:
            shape_id = int(raw_id)
        except ValueError as exc:
            raise SvgNativeConversionError(
    ...
    if xml.startswith("<mc:AlternateContent"):
        if elem.tag.replace(f"{{{SVG_NS}}}", "") == "text":
            inline_formula_count = sum(
                1 for child in elem.iter() if child.get(INLINE_FORMULA_ATTR) is not None
            )
            return {
    ...
    *,
    ...
    def walk(
        ...
        is_supported_visual_child = (
            tag in _SUPPORTED_VISUAL_CHILD_TAGS and parent_tag in {"text", "tspan", "a"}
        )
        is_data_icon_placeholder = (
            allow_data_icon_use and tag == "use" and elem.get("data-icon") is not None
        )
        if (
            tag not in _CONVERTERS
...
def convert_svg_to_slide_shapes(
    ...
    inline_formula_count_before_text_lowering = sum(
        1 for elem in root.iter() if elem.get(INLINE_FORMULA_ATTR) is not None
    )
    hyperlink_count_before_text_lowering = sum(
        1
        for elem in root.iter()
        if (elem.tag == f"{{{SVG_NS}}}a" or elem.get(SHAPE_HYPERLINK_ATTR) is not None)
    )
    ...
    has_top_level_group = any(
        child.tag.replace(f"{{{SVG_NS}}}", "") == "g" for child in root
    )
    background_xml, background_skip_id = (
        _extract_background_candidate(root, ctx) if promote_background else ("", None)
    )
    ...
    for child in root:
        ...
        if result:
            ...
            structurally_static = child.get("data-pptx-layer") is not None or (
                has_explicit_semantics and is_static_page_frame(role, placeholder)
            )
            legacy_chrome = not has_explicit_semantics and is_chrome_id(elem_id)
            explicit_legacy_override = (
                elem_id is not None and elem_id in ctx.animation_group_overrides
            )
            if (
                m
