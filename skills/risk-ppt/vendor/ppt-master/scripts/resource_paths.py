...
_SVG_EXTERNAL_DOCTYPE_RE = re.compile(
    rb"<!DOCTYPE\b[^>]*\b(?:PUBLIC|SYSTEM)\b",
...
def project_root_for_svg_path(svg_path: Path) -> Path:
    ...
    if base.name in SVG_WORK_DIR_NAMES or base.name.startswith(
        SVG_FINAL_CANDIDATE_PREFIX
    ):
        return base.parent
    if (
...
def _decode_svg_data_uri(raw: str) -> tuple[bytes | None, str | None]:
    ...
    is_base64 = any(
        token.strip().lower() == "base64" for token in header.split(";")[1:]
    )
    try:
        decoded = (
