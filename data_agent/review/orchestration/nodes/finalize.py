...
def finalize(state: ParentState, config: RunnableConfig) -> dict:
    ...
    has_gaps = bool(blocked) or any(entry.status == "unsupported" for entry in coverage)
    terminal_status = RunStatus.COMPLETED_WITH_GAPS if has_gaps else RunStatus.COMPLETED
    run = ReviewRun(
    ...
    return {"status": terminal_status.value, "final_markdown": markdown}
